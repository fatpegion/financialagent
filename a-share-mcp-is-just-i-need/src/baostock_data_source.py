...
class BaostockDataSource(FinancialDataSource):
    ...
    # 新增爬虫功能（简化版，风险/情感分析由大模型微调处理）
    def crawl_news(self, query: str, top_k: int = 10) -> str:
        """
        从百度新闻搜索相关文章并返回格式化结果。

        注：风险分析和情感分析已移除，由大模型微调负责。
        """
        import urllib.parse
        import re as regex
        ...
        try:
            ...
            if '百度安全验证' in response.text:
                logger.warning("百度安全验证触发，尝试备用方案")
                return "抱歉，百度搜索触发了安全验证，无法获取搜索结果。"
            ...
            for title_elem in title_elements[:top_k * 2]:
                try:
                    ...
                    if not title or len(title) < 3:
                        continue
                    if any(skip in title for skip in ['官方网站', '百度百科']):
                        continue

                    if link.startswith('/link?url='):
                        try:
                            actual_url = regex.search(r'url=([^&]+)', link)
                            if actual_url:
                    ...
                    abstract = ''
                    if parent:
                        abstract_elem = parent.find(['div', 'span'], class_=lambda x: x and ('abstract' in str(x).lower() or 'content' in str(x).lower() or 'summary' in str(x).lower()) if x else False)
                        if abstract_elem:
                    ...
                    results.append({
                        'title': title,
                        'content': full_content[:500] if full_content else abstract,
                        'link': link,
                        'source': '百度新闻',
                        'date': '未知',
                    })
