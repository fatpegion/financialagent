...
def baostock_login_context():
    ...
    使用 contextlib.redirect_stdout 来抑制输出，跨平台兼容（Windows/Linux/macOS）。
    支持最多3次重试，处理网络不稳定情况。
    """
    f = io.StringIO()
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            with redirect_stdout(f):
                lg = bs.login()
            logger.debug(f"Login result (attempt {attempt+1}): code={lg.error_code}, msg={lg.error_msg}")

            if lg.error_code == '0':
                logger.info("Baostock login successful.")
                break
            elif lg.error_code == '10002007':
                logger.warning(f"Baostock network error (attempt {attempt+1}/{max_retries}), retrying...")
            ...
            logger.warning(f"Redirect failed on attempt {attempt+1}: {e}")
            if attempt == max_retries - 1:
                try:
                    lg = bs.login()
                    if lg.error_code != '0':
                    ...
                    logger.error(f"Direct Baostock call failed: {inner_e}")
                    raise LoginError(f"Baostock login failed: {inner_e}")

    if lg.error_code != '0':
        raise LoginError(f"Baostock login failed after {max_retries} attempts: {lg.error_msg}")
